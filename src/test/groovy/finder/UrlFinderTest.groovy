...
abstract class UrlFinderTest extends Specification {
    ...
    def "should find http Url in String containing only url in quotations"() {
        given:
        def onlyUrlWithQuotations = '"/wiki/Some_site"'
        ...
        then:
        urls.size == 1
        urls.first() == new URL("https://pl.wikipedia.org/wiki/Some_site")
    }

    def "should NOT find http Url in String containing only url WITHOUT quotations"(){
        given:
        def onlyUrl = '/wiki/Some_site'
    ...
    def "should find three http Urls in String containing them and not Urls"() {
        ...
        then:
        urls.size == 3
        urls[0] == new URL("https://pl.wikipedia.org/wiki/example")
        urls[1] == new URL("https://pl.wikipedia.org/wiki/Some_site")
        urls[2] == new URL("https://pl.wikipedia.org/wiki/test")
    }

    //TODO test not searching for Files and links with colon
    //TODO ensure that Polish signs are working
